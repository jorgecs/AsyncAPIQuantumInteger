...
const mergeFiles = require('./parameters/mergefiles');
...
{% if params.quantumInteger -%}
mergeFiles.mergefiles('{{ quantum.quantum_url }}', './quantumFile{{channel.publish().id()}}.py');
{% else -%}
...
{% if params.quantumInteger -%}
mergeFiles.mergefiles('{{ quantum.quantum_url }}', './quantumFile{{channel.subscribe().id()}}.py'); 
{% else -%}
fetch('{{ quantum.quantum_url }}')
  .then(res => res.text())
  .then(data => {
